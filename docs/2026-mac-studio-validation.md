# 2026 Mac Studio profile validation

## Known hardware

- Apple announcement: August 25, 2026.
- Customer availability: September 22, 2026.
- `Mac17,14`: Mac Studio with M5 Max.
- `Mac17,15`: Mac Studio with M5 Ultra.
- The enclosure and front status LED retain the previous Mac Studio design.
...
- Candidate profiles exist only under `unverified` in the private, git-ignored
  `profiles.local.json`.
- The M5 Max candidate inherits the M4 Max profile.
- The M5 Ultra candidate inherits the M3 Ultra profile.
...
2. Record the chip name and model identifier without collecting a serial number
...
6. Repeat on both M5 Max and M5 Ultra hardware; do not infer one result from the
   other.
...
9. Move the validated identifier from `PENDING_EXACT` to `SUPPORTED_EXACT`, run
   the full test suite, sync the auditable public client, and publish a new
   signed release.
